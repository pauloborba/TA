...
Given(~/^that report with name "([^"]*)", evaluation "([^"]*)", type "([^"]*)", value "([^"]*)" is in the system$/) {
    String repoName, String repoEval, String  repoType, double repoVal->
...
When(~'^I add the evaluation "([^"]*)" in the criterion "([^"]*)" at turma "([^"]*)" and periodo "([^"]*)" with origin "([^"]*)" and date "([^"]*)" to the student with name "([^"]*)" and the login "([^"]*)"$'){
    String eval, String criteName, String classID, String periodo, String origin, String dat, String nomeA, String loginA ->
        ...
        CriterionTestDataAndOperations.createCriterion(criteName, t.id.toString())
...
Then(~'^70% of the student "([^"]*)" evaluations are composed of "([^"]*)"$'){
    String loginA, String evalType->
       // assert ReportsDataAndOperations.checkCondition(loginA, evalType)
}


And(~/^the report "([^"]*)" is updated adding the student with login "([^"]*)"$/) {
    String repoName, String loginA ->
...
And(~/^I try to create the report named "([^"]*)", type "([^"]*)", value "([^"]*)", evaluation "([^"]*)"$/) {
    String arg1, String arg2, double arg3, String arg4 ->
    nomeReport = arg1
    tipoReport = arg2
    valorReport = arg3
...
When(~/^I select the "([^"]*)" report$/) {
    String arg1 ->
        at IndexReport
        page.selectReport(arg1)
}
Then(~/^I should see the details related to the "([^"]*)" report$/) {
    String arg1 ->
//    at ShowReportsPage
