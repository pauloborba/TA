...
When(~/^I add a class with ID "([^"]*)" and periodo "([^"]*)"$/) { String classID, String periodo ->
    ClassTestDataAndOperations.createClass(classID, periodo)
    t = ClassTestDataAndOperations.getTurma(classID, periodo)
    assert t != null
}
Then(~/^the class "([^"]*)" with periodo "([^"]*)" is properly stored in the system$/) { String classID, String periodo ->
    assert ClassTestDataAndOperations.compatibleTo(classID, periodo, t)
}
...
When(~/^I fill the class details with name "([^"]*)", periodo "([^"]*)"$/) {
    String classID, String periodo ->
        at CreateTurmaPage
        page.fillTurmaDetails(classID, periodo)
}
...
And(~/^I can see the information for class "([^"]*)", periodo "([^"]*)" at the Turma Listagem page$/) {
    String classID, String periodo ->
        to TurmasPage
        at TurmasPage
