Feature: Send email with evaluated criterion
  ...
  Scenario: Send email to all students about all evaluation criterion
    Given the student "Aimée" login "aacb" and the criterion "Test implementation" are registered in the system
  ...
  Scenario: notify that there is no evaluated criterion
    Given I see the student "Pedro" login "plal" and the criterion "Requirements definition"
  ...
  Scenario: Check if the student continues in the system
    Given the student "João" login "J" is in the system
    When I request to send an email with evaluated criterion
    Then the student "João" is still in the system