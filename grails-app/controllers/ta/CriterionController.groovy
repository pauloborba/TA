...
class CriterionController {
    ...
    def getStudents(Long id) {
        Criterion criterion = Criterion.findById(id)
        List<Turma> turmas = criterion.turma.students
        def res = turmas.collect {['name':it.name, 'login':it.login]}

        render res as JSON
    }

    def getStudentsList(Long id){
        Criterion criterion = Criterion.findById(id)
        List<Student> estudantes = criterion.turma.students
        def res = estudantes.collect {['id':it.id, 'name':it.name, 'login':it.login]}
        render(template: "students", collection: res, var: "student")
    }
    ...
    def saveGroup(){
        ...
        String turma = params.turma
        String[] criteria = group.split(";")
        for (int i = 0; i < criteria.size(); i++) {
            Criterion novo = new Criterion(criteria[i], turma)
