...
//        StudentController student = new StudentController()
        ...
        Criterion criterion = Criterion.findById(params.criterion.id)
        List<Student> list = criterion.turma.students
        for(int i = 0; i < list.size(); i++){

            String studentId = list[i].id.toString()
            String fieldname = "value_"+studentId
            String value = params.get(fieldname)

            Score novo = new Score(value, studentId, (String)newEvaluation.id)
