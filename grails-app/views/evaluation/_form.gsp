...
<div class="fieldcontain ${hasErrors(bean: evaluationInstance, field: 'criterion', 'error')} required">
	...
	<g:select id="criterion" name="criterion.id" from="${ta.Criterion.list()}" optionKey="id" required="" optionValue="${{it.turma.classID+' - '+it.turma.periodo+' - '+it.description}}" value="${evaluationInstance?.criterion?.id}" class="many-to-one"/>
